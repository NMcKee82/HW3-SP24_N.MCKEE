def Simpson2(fcn, args, a, b, npoints = 21):
    ...
    '''
    area = 0 #inital value for the integral
    ...
    h = (xR-xL)/(2*m) #calculate panel widthg
    x = xL
    area = (fcn((xL, args))+fcn((xR, args))) #first calculate f(x) at xL and xR

    for j in range(1,2*m):_#update counts from 1 to 2*m-1
            x = j*h+xL # update the x position for evaluating the function
        if not j % 2 == 0: #the odds
            area += 4*fcn((x, args))
        else: #the evens
            area += 2*fcn((x, args))
    return (h/3,0)*area # finally, returns the value for the integral